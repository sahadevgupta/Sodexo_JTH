...
namespace Sodexo_JTH.Controls
{
    public class ObservableRangeCollection<T> : ObservableCollection<T>
    {
        ...
        public virtual void FindAndRemove(Func<T, bool> Match, NotifyCollectionChangedAction notificationMode = NotifyCollectionChangedAction.Remove)
        {
            ...
            var collection = this.Items.Where(Match);
            if (collection is IEnumerable<T> list && list.Count() == 0) return;
            else if (!collection.Any()) return;
